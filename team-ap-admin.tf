...
module "security_restricted_admin" {
  source = "./modules/assume_role"
  tags   = local.tags

  destination_role_name    = "restricted-admin"
  destination_account_name = "security"
  user_names               = module.analytical_platform_team.user_names
  user_arns                = module.analytical_platform_team.user_arns
...
output "security_restricted_admin" {
  value = module.security_restricted_admin.destination_role.name
}
